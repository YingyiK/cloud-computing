...
def lambda_handler(event, context):
    ...
    try:
        # Calculate timestamp for last 10 seconds
        current_timestamp = int(time.time())
        start_timestamp = current_timestamp - 10
        
        # Query bucket size history for the last 10 seconds
        history_data = query_bucket_size_history(BUCKET_NAME, start_timestamp)
        print(f"Found {len(history_data)} data points in the last 10 seconds")
